...
-- ==========================
-- 3. TECHNICAL STAFF (kế thừa STAFF)
-- ==========================
CREATE TABLE tblTechnicalStaff (
    tblStaffId INT PRIMARY KEY,
    skills VARCHAR(255),
    FOREIGN KEY (tblStaffId) REFERENCES tblStaff(tblUserId)
        ON DELETE CASCADE ON UPDATE CASCADE
);

-- ==========================
-- 3.1 WAREHOUSE STAFF (kế thừa STAFF)
-- ==========================
CREATE TABLE tblWarehouseStaff (
    id INT AUTO_INCREMENT PRIMARY KEY,
    tblUserId INT UNIQUE NOT NULL,
    FOREIGN KEY (tblUserId) REFERENCES tblUser(id)
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE tblInvoice (
    ...
    tblCustomerId INT,
    tblSalesStaffId INT,
    FOREIGN KEY (tblCustomerId) REFERENCES tblUser(id)
        ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (tblSalesStaffId) REFERENCES tblStaff(tblUserId)
        ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE tblServiceRequest (
    ...
    tblRequestId INT,
    tblServiceId INT,
    tblTechnicalStaffId INT,
    FOREIGN KEY (tblRequestId) REFERENCES tblRequest(id)
        ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (tblServiceId) REFERENCES tblService(id)
        ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (tblTechnicalStaffId) REFERENCES tblTechnicalStaff(tblStaffId)
        ON DELETE CASCADE ON UPDATE CASCADE
);
